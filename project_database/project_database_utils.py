...
class TestUtil:
    ...
    def measure_seconds_performance_nth(function):
        ...
        # pk_print(f"{constants.UNDERLINE_PROMISED}TEST TRYING..."), FAIL
        ...
        def wrapper(**kwargs):  # **kwargs, keyword argument, dictionary 로 parameter 를 받음. named parameter / positional parameter 를 받을 사용가능?
            ...
            if TestUtil.is_first_test_lap:
                pk_print(f"{constants.UNDERLINE_PROMISED}TEST READY")
                ment = rf"총 {n}번의 시간성능측정 테스트를 시도합니다"
                pk_print(ment)
                speak_ment(ment=ment, sleep_after_play=1)
                TestUtil.is_first_test_lap = False
                pk_print(f"{constants.UNDERLINE_PROMISED}TEST START")
            seconds_performance_test_results = TestUtil.test_results
            import time
            time_s = time.time()
            try:
                pk_print(f"{constants.UNDERLINE_PROMISED}TEST LOOP {len(seconds_performance_test_results)} STARTED")
                # function(*args, **kwargs)
                function(**kwargs)
                # function(*args)
                # function()
                pk_print(f"{constants.UNDERLINE_PROMISED}TEST LOOP {len(seconds_performance_test_results)} ENDED")
                # n = n +1
            except:
                pk_print("%%%FOO%%%")
            ...
            if len(seconds_performance_test_results) == n:
                ...
                speak_ment(ment=ment, sleep_after_play=0.55)
                ...
                pk_print(ment)
                pk_print(f"{constants.UNDERLINE_PROMISED}TEST END")
                TestUtil.pause()

            # api 테스트 시 여러번 빠르게 api 요청 시 거절 되는 것 같음. 이를 해결하기 위한 코드. 불필요 시 주석, api 제공할 때 어떤방법을 써서 3 번까지만 허용시켜둔 것 같다.
            pk_sleep(milliseconds=random.randint(4000, 5100), print_mode=True)
...
class PkProgramPerformanceOptimizingUtil:
    ...
    def gen_dictionary_for_monitor_target_edited_and_back_up(directory_abspath):
        ...
        """
        current_directory_state = get_directory_files_files_for_monitoring_mtime_without_files_to_exclude(directory_abspath=directory_abspath)  # str to dict
        ...
        tree_levels = [get_tree_depth_level(sample) for sample in current_target_files]  # 파일시스템 트리깊이레벨 목록
...
class DbTomlUtil:
    ...
    def read_db_toml():
        try:
            ...
            return toml.load(constants.DB_TOML)
        except:
            pk_print("%%%FOO%%%")
    ...
    #     try:
    #         pk_print(f"{inspect.currentframe().f_code.co_name}()")
    #         back_up_target(constants.DB_TOML)
    #     except:
    #         pk_print("%%%FOO%%%")
    ...
    def delete_db_toml():
        try:
            pk_print(f"{inspect.currentframe().f_code.co_name}()")
            convert_as_zip_with_timestamp(constants.DB_TOML)
        except:
            pk_print("%%%FOO%%%")
    ...
    def is_accesable_local_database():
        pk_print(f"{inspect.currentframe().f_code.co_name}()")
        if not os.path.exists(constants.DB_TOML):
...
#         if previous_input == "":
#             previous_input = constants.SERVICE_DIRECTORY
...
#             if os.path.exists(constants.MACRO_LOG):
#                 move_target_to_trash_bin(constants.MACRO_LOG)
#             make_pnx(mode='f',(constants.MACRO_LOG)
#             explorer(constants.MACRO_LOG)
...
#                 # 랜덤보상들 여기에 작성하기
#                 explorer(constants.MUSIC_FOR_WORK)
#                 break
...
#                     useless_directories = constants.USELESS_DIRECTORIES
#                     make_d_leaf(useless_directories)
#                     useless_files = []
#                     is_target_moved_done = False
#                     lines = get_lines_of_file(constants.USELESS_FILES)
#                     tmp = clipboard.paste()
...
#                         # destination_directory_of_files_classified = constants.SPECIAL_DIRECTORY # e: drive 에 저장
...
#     pk_print(f"{inspect.currentframe().f_code.co_name}()")
#     constants.count_of_make_me_go_to_sleep.append("!!")
...
#     if len(constants.count_of_make_me_go_to_sleep) == 10:
...
def pk_sleep(milliseconds=None, sec=None, min=None, hour=None, print_mode=True):
    # pk_print(f"{inspect.currentframe().f_code.co_name}()")
    params_list = [milliseconds, sec, min, hour]
    # 리스트 내 요소가 3개만 None 인지 확인
    if get_none_count_of_list(params_list) == 3:
        # milliseconds = None, sec = None, min = None, hour = None, 이 중 하나만 None 이 아니면 동작
        time_to_sleep = None
        if milliseconds is not None:
            seconds = milliseconds / 1000
            if print_mode:
                pk_print(f"{inspect.currentframe().f_code.co_name}(ms={milliseconds})")
            time_to_sleep = seconds
        elif sec is not None:
            if print_mode:
                pk_print(f"{inspect.currentframe().f_code.co_name}(seconds={sec})")
            time_to_sleep = sec
        elif min is not None:
            if print_mode:
                pk_print(f"{inspect.currentframe().f_code.co_name}(minutes={min})")
            time_to_sleep = 60 * min
        elif hour is not None:
            if print_mode:
                pk_print(f"{inspect.currentframe().f_code.co_name}(hours={hour})")
            time_to_sleep = 60 * 60 * hour
        if time_to_sleep is not None:
            time.sleep(time_to_sleep)
    else:
        pk_print(f'sleep() 메소드는 milliseconds, sec, min, hour 중 하나만 단위만 설정할 수 있습니다')
...
#             break
#         # 다운로드가 안되면 주석 풀어 시도
#         # os.system(rf'{constants.YT_DLP_CMD} -U')
#
...
#         print('다운로드 옵션 파싱 중...')
#         video_id = ''
#         # lines = subprocess.check_output(rf'{constants.YT_DLP_CMD} -F {url}', shell=True).decode('utf-8').split("\n")
#
#         cmd = rf'{constants.YT_DLP_CMD} -F {url}'
...
#         # 차선책으로 두는 것이 낫겠다
#         # cmd = rf'{constants.YT_DLP_CMD} -f best "{url}"'
#
#         # cmd = rf'{constants.YT_DLP_CMD} -f {video_id}+{audio_id} {url}' # 초기에 만든 선택적인 방식
#         # cmd = rf'{constants.YT_DLP_CMD} -f "best[ext=webm]" {url}' # 마음에 안드는 결과
#         cmd = rf'{constants.YT_DLP_CMD} -f "bestvideo[ext=webm]+bestaudio[ext=webm]" {url}'  # 지금 가장 마음에 드는 방법
#         # cmd = rf'{constants.YT_DLP_CMD} -f "bestvideo[ext=webm]+bestaudio[ext=webm]/best[ext=webm]/best" {url}' # 아직 시도하지 않은 방법
...
#                 dialog.exec()
#                 pk_print(f"{constants.UNDERLINE_PROMISED}다운로드 가능한 video_id 와 audio_id 를 가용목록에 추가해주세요")
#                 pk_print(cmd)
#                 break
#
#             if src != os.getcwd():  # 여기 또 os.getcwd() 있는 부분 수정하자..
...
#                     pk_print(f'{constants.UNDERLINE_PROMISED} youtube video id parsing mode')
#                     try:
...
#                         continue
#                 else:
#                     pk_print(f'{constants.UNDERLINE_PROMISED} experimental mode')
#                     print("???????????????????")
#                     try:
#                         url_parts_useless = [
#                             "https://youtu.be/",
...
def update_global_pkg_pk_server_fastapi_for_linux():
    ...
    try:
        if os.path.exists(global_pkg):
            ...
            pk_print(f'{cmd}')
            pk_print(f"{constants.UNDERLINE_PROMISED}")
...
def merge_video_and_sound(file_v_abspath, file_a_abspath):
    ...
    location_ffmpeg = rf"{constants.D_HOME}\Desktop\`workspace\tool\LosslessCut-win-x64\resources\ffmpeg.exe"
...
#     pk_print(f"{inspect.currentframe().f_code.co_name}()")
#     GIT_HUB_ADDRESS = constants.GIT_HUB_ADDRESS
...
def save_all_list():  # 수정 필요, 모든 파일 디렉토리 말고, 특정디렉토리로 수정한는 것도 할것
    ...
    pk_print(f"{constants.UNDERLINE_PROMISED}{constants.UNDERLINE_PROMISED} all_list.txt writing e")
    pk_print(f"{constants.UNDERLINE_PROMISED}{constants.UNDERLINE_PROMISED} all_list_proper.txt rewriting s")
    texts_black = [
        "C:\\$WinREAgent",
        "C:\\mingw64",
        "C:\\PerfLogs",
    ...
    f.close()
    f2.close()
    pk_print(f"{constants.UNDERLINE_PROMISED}{constants.UNDERLINE_PROMISED} all_list_proper.txt rewriting e")

    pk_print(f"{constants.UNDERLINE_PROMISED}{constants.UNDERLINE_PROMISED} files opening s")
    os.chdir(os.getcwd())
    ...
    pk_print(f"{constants.UNDERLINE_PROMISED}{constants.UNDERLINE_PROMISED} files opening e")

    # os.system('del "'+os.getcwd()+'\\all_list.txt"')
    # mk("all_list.txt")
    pk_print(f"{constants.UNDERLINE_PROMISED}{constants.UNDERLINE_PROMISED} e")
...
#                                 pk_print(f"{constants.UNDERLINE_PROMISED}다운로드 가능한 범위의 숫자인덱스입니다")
#                         except IndexError:
...
#         else:
#             pk_print(f"{constants.UNDERLINE_PROMISED}프로세스가 실행 중이지 않습니다.")
#     except psutil.NoSuchProcess:
#         pk_print(f"{constants.UNDERLINE_PROMISED}유효하지 않은 PID입니다.")
...
#     for biggest_target in constants.biggest_targets:
...
#     for target in constants.smallest_targets:
...
def classify_targets_between_smallest_targets_biggest_targets():
    ...
    targets = [
        rf"{constants.D_HOME}\Desktop\services\helper-from-youtube-url-to-webm",
        rf"{constants.D_HOME}\Desktop\services",
    ]
    ...
    for target in targets:
        ...
        if target_size_megabite <= 300:
            constants.smallest_targets.append(target.strip())

        elif 300 < target_size_megabite:
            constants.biggest_targets.append(target.strip())
        else:
            pk_print(f'{target.strip()}pass')

    pk_print(f'smallest_target 출력')
    # targets 에서 biggest_targets 과 일치하는 것을 소거 시도
    smallest_targets = [i for i in targets if i not in constants.biggest_targets]
    for target in constants.smallest_targets:
        print(target)

    pk_print(f'biggest_target 출력')
    for target in constants.biggest_targets:
        print(target)
    pass
...
def gather_storages():
    pk_print(f"{inspect.currentframe().f_code.co_name}()")
    starting_directory = os.getcwd()
    dst = constants.DIRECTORYIES_GATHERED
    if not os.path.exists(dst):
        return
    services = os.path.dirname(dst)
    os.chdir(services)
    storages = []
    cmd = rf'dir /b /s "{constants.D_HOME}\Downloads"'
...
def run_targets_promised():
    ...
    try:
        targets = [
            ...
            constants.F_PYCHARM64_EXE,
            D_PROJECT,
            constants.SERVICE_DIRECTORY,
        ]
        for target in targets:
            explorer(target)
    except:
...
#
#             should_i_check_your_promised_routines_before_coding(constants.routines_promised)
#
...
#             monitor_target_edited_and_back_up(target_abspath=constants.pk_server_fastapi_ARCHIVE_TOML)  # seconds_performance_test_results : ['11.95sec', '26.78sec', '11.94sec', '3.7sec', '11.72sec']
...
#                 monitor_target_edited_and_sync(target_abspath=constants.SERVICE_DIRECTORY)  # seconds_performance_test_results : ['28.46sec', '27.53sec', '2.85sec', '2.9sec', '2.91sec']
#
#         # 0시에서 4시 사이, 30초 마다
...
#     pk_print(f"{constants.UNDERLINE_PROMISED} {inspect.currentframe().f_code.co_name}")
...
#             if json_str != None:
#                 # lines = get_cmd_output(cmd=rf'echo "{json_str}" | {constants.JQ_WIN64_EXE} "."') # 나오긴 하는데 한줄로 나온다
...
#                 pk_print(json_str)
#             if json_file != None:
#                 lines = get_cmd_output(cmd=rf"type {json_file} | {constants.JQ_WIN64_EXE} ")
#                 [pk_print(line) for line in lines]
#             if json_list != None:
#                 lines = get_cmd_output(cmd=rf'echo "{json_list}" | "{constants.JQ_WIN64_EXE}" ')
#                 [pk_print(line) for line in lines]
#         else:
...
#     text_promised = constants.SERVICE_DIRECTORY
#     while 1:
...
def gather_empty_directory(target_abspath: str):
    ...
    # 빈 디렉토리 제거
    make_d_leaf(leaf_directory_abspath=constants.D_EMPTY)
    if os.path.isdir(target_abspath):
        for dirpath, dirnames, filenames in os.walk(target_abspath, topdown=False):
            if not dirnames and not filenames:
                if os.path.isdir(dirpath):
                    is_empty_directory(dirpath)
                    move_target_without_overwrite(target_abspath=dirpath, dst=constants.D_EMPTY)
            ...
            for root, directories, files in os.walk(directory, topdown=True):
                for directory in directories:
                    ...
                    if is_leaf_directory(directory):
                        move_target_without_overwrite(target_abspath=directory, dst=constants.D_EMPTY)
...
def merge_excel_files(dir_path):
    ...
    try:
        ...
        # 엑셀 병합 및 저장
        merged_file = constants.FILE_MERGED_EXCEL_XLSX
        merged_cnt = 0
...
def convert_mp4_to_webm(target_abspath):
    ...
    os.chdir('storage')
    os.system(f'"{constants.FFMPEG_EXE}" -i "{target_abspath}" -f webm -c:v libvpx -b:v 1M -acodec libvorbis "{file_edited}" -hide_banner')
...
def convert_wav_to_flac(target_abspath):
    ...
    ffmpeg_exe = rf"{constants.D_HOME}\Desktop\`workspace\tool\LosslessCut-win-x64\resources\ffmpeg.exe"
    destination = 'storage'
    try:
...
#         else:
#             remove_target_parmanently(constants.DIRSYNC_LOG)
#             # 로깅 설정 및 pk_server_fastapi.DIRSYNC_LOG 생성
#             import logging
#             logging.basicConfig(filename=constants.DIRSYNC_LOG, level=logging.DEBUG, filemode='w', encoding='utf-8')
...
#                 if os.path.exists(constants.DIRSYNC_LOG):
#                     lines = get_lines_of_file(constants.DIRSYNC_LOG)[-4:-1]
...
def get_directory_files_files_for_monitoring_mtime_without_files_to_exclude(directory_abspath):
    files_to_exclude = [
        constants.DB_TOML,
        constants.F_SUCCESS_LOG,
        constants.LOCAL_PKG_CACHE_FILE,
        constants.DIRSYNC_LOG,
    ]
    files_of_directory = []
...
def move_target_to_trash_bin(target_abspath):
    ...
    # 휴지통 열기
    # pk_server_fastapi.get_cmd_output('explorer.exe shell:RecycleBinFolder')
...
def get_length_of_mp3(target_abspath: str):
    ...
    try:
        ...
        # pk_print("%%%FOO%%%") # gtts 모듈 불능? mutagen 모듈 불능? license 찾아보자 으로 어쩔수 없다.
        return
    except Exception:
        pk_print("%%%FOO%%%")
...
def run_loop_for_speak_as_async_deprecated(ment):
    async def speak_as_async(ment):
        ...
        pk_print(rf'before_mp3_length_used_in_speak_as_async 만큼 재생 대기')

        # pk_server_fastapi.pyglet_player 을 del 을 한 경우:
        ...
        if constants.pyglet_player == None:
            constants.pyglet_player = pyglet.media.Player()
        # if hasattr(pk_server_fastapi_, 'pyglet_player') and pk_server_fastapi.pyglet_player is None:
        #     # pk_server_fastapi.pyglet_player가 None인 경우
        #     pk_server_fastapi.pyglet_player = pyglet.media.Player()
        #     pass
        if constants.pyglet_player != None:
            if constants.pyglet_player.playing == True:
                length_of_mp3 = constants.PREVIOUS_MP3_LENGTH_USED_IN_SPEAK_AS_ASYNC
                ...
                if type(ment) == str:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # pk_server_fastapi.multiprocessed_source_play = source
                            ...
                            playing_sounds = constants.playing_sounds
                            playing_sounds.append(pyglet_player)
                            # 사운드 재생을 시작할 때마다 이벤트 핸들러를 등록하여 playing_sounds 리스트에 추가

                            # FAIL
                            # import  multiprocessing
                            # pk_server_fastapi.multiprocessed_source_play = multiprocessing.Process(target= source.play , args=None)
                            # pk_server_fastapi.multiprocessed_source_play.start()

                            if constants.pyglet_player.playing == False:
                                constants.pyglet_player = None
